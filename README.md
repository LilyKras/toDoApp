...
14. Для стейт-менеджмента используются Stateful виджеты и provider (могут быть задержки при сохранении заметки - сделано специально, чтобы потом можно было обработать ошибки) в этом месте.
15. Реализована работа с бэкендом, данные отправляются/получаются с сервера. Работа с сетью выделена в отдельный слой - ОШИБКИ НЕ ОБРАБОТАНЫ, ЭТО ЧАСТЬ СЛЕДУЮЩЕГО ДЗ!!!
ТАК ЖЕ НЕ НАЛАЖЕНА РАБОТА В ОФФЛАЙНЕ - ТОЖЕ ЧАСТЬ СЛЕДУЮЩЕГО ДЗ!!!
